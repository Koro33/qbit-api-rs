...
async fn main() -> Result<()> {
    ...
    //// add torrent by url
    let torrent_urls = ["magnet:?xt=urn:btih:TZRYKYVLDQP45WO66FBIMTG5LJYBTYNK&dn=ubuntu-22.04.2-live-server-amd64.iso&xl=1975971840&tr=https%3A%2F%2Ftorrent.ubuntu.com%2Fannounce"];
    ...
    // let torrent_files = ["tests/ubuntu-22.04.2-live-server-amd64.iso.torrent"];
    ...
    //// pause torrents
    let hashes = ["9e638562ab1c1fced9def142864cdd5a7019e1aa".to_owned()];
    let _ = qb_cli.torrents_pause(&hashes).await?;
    hashes
        .iter()
    ...
    //// resume torrents
    let hashes = ["9e638562ab1c1fced9def142864cdd5a7019e1aa".to_owned()];
    let _ = qb_cli.torrents_resume(&hashes).await?;
    hashes
        .iter()
        .for_each(|h| log::info!("torrent resumed: {:?}", h));

    //// delete torrents
    let hashes = vec!["9e638562ab1c1fced9def142864cdd5a7019e1aa".to_owned()];
    let _ = qb_cli.torrents_delete(&hashes, true).await?;
    hashes
        .iter()
